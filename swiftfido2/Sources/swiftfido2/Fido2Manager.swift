...
public class Fido2Manager {
  func open(withHidDevice deviceInfo: FidoDeviceInfo) throws -> FidoDeviceContext {
    ...
    let inLen = try ReportLengthUtility.getReportLength(device: device, direction: .inward)
    let outLen = try ReportLengthUtility.getReportLength(device: device, direction: .outward)
