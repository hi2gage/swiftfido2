...
struct MakeCredentialResponse {
    ...
    init(from resp: Ctap2Response) throws {
        ...
        guard case let .utf8String(fmt)? = resp.map[.unsignedInt(1)] else {
            throw Ctap2Error.missingField("fmt")
        }
        self.fmt = fmt

        guard case let .byteString(ad)? = resp.map[.unsignedInt(2)] else {
            throw Ctap2Error.missingField("authData")
        }
        self.authData = Data(ad)

        guard case let .map(stmt)? = resp.map[.unsignedInt(3)] else {
...
struct GetAssertionResponse {
    ...
    init(from resp: Ctap2Response) throws {
        ...
        // 1) credentialID
        guard case let .map(credMap)?       = map[.unsignedInt(1)],
              case let .byteString(idBytes)? = credMap[.utf8String("id")]
        else {
        ...
        // 2) authData
        guard case let .byteString(ad)? = map[.unsignedInt(2)] else {
          throw FieldError.missingField("authData")
        }
        authData = Data(ad)

        // 3) signature
        guard case let .byteString(sig)? = map[.unsignedInt(3)] else {
          throw FieldError.missingField("signature")
        }
        signature = Data(sig)

        // 4) userHandle (optional)
        if case let .map(userMap)? = map[.unsignedInt(4)],
           case let .byteString(uhBytes)? = userMap[.utf8String("id")] {
        ...
        if case let .unsignedInt(n)? = map[.unsignedInt(5)] {
...
struct GetInfoResponse {
    ...
    init(from resp: Ctap2Response) throws {
        ...
        guard case let .array(vs)? = resp.map[.unsignedInt(1)] else {
            throw Ctap2Error.missingField("versions")
        }
        versions = vs.compactMap { if case let .utf8String(s) = $0 { return s } else { return nil } }

        if case let .array(exts)? = resp.map[.unsignedInt(2)] {
            extensions = exts.compactMap { if case let .utf8String(s) = $0 { return s } else { return nil } }
        } else {
            extensions = nil
        }

        guard case let .byteString(ag)? = resp.map[.unsignedInt(3)], ag.count == 16 else {
            throw Ctap2Error.missingField("aaguid")
        }
        aaguid = Data(ag)

        guard case let .map(opts)? = resp.map[.unsignedInt(4)] else {
            throw Ctap2Error.missingField("options")
        }
        options = opts.reduce(into: [:]) { dict, pair in
            if case let .utf8String(key) = pair.key,
               case let .boolean(val) = pair.value {
                dict[key] = val
            }
        }

        guard case let .unsignedInt(max)? = resp.map[.unsignedInt(5)] else {
            throw Ctap2Error.missingField("maxMsgSize")
        }
        maxMsgSize = max

        if case let .array(pps)? = resp.map[.unsignedInt(6)] {
            pinProtocols = pps.compactMap {
                if case let .unsignedInt(p) = $0 { return p }
                else { return nil }
            }
        } else {
            pinProtocols = nil
